        ...
        <div class="col-12 grid-margin">
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- Header -->
                    <headerComponent        :title="'List of Brands'"       :add_button_page="'New Brand'"  :update_button_page="'Update Brand'"/>

                    <!-- HeaderPaging -->
                    <headerRouterComponent  :produits_button="'show'"           :marques_button="'show'"            :categories_button="'show'"     :types_button="'show'" />

                    <!-- Table -->
                    <table class="table table-bordered clickable_table" id="marque_index">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <tr v-for="marque, index in marques" :key="index" @click="selectRow(marque)"   role="button"   :id="'marque_index_'+marque.id">
                                <td>{{index + 1}}</td>
                                <td>{{marque.nom}}</td>
                                <td>{{marque.description}}</td>
                                <td>{{marque.libelle_route_import}}</td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    ...
    data() {
        return {
            marques                   :   [],
            datatable_marque_index    :   null,

            selected_marque           :   null,
            display_modal           :   false,
    ...
    methods : {
        ...
        async setDataTable() {
            ...
            try {
                ...
                // Destroy DataTable
                if(this.datatable_marque_index)  {

                    this.datatable_marque_index.destroy()
                }

                // Initialisation 
                this.marques   =   [];

                this.$callApi("post",    "/marques", null)
                .then(async (res)=> {
                    ...
                    this.datatable_marque_index   =   await this.$DataTableCreate("marque_index")
                })
            }
        ...
        async addElementPage() {
            ...
            this.$router.push("/marques/add")
        },

        async updateElementPage() {

            this.$router.push("/marques/"+this.selected_row.id+"/update")
        },
        ...
        selectRow(marque) {
            ...
            // Get Element
            const row           =   document.getElementById("marque_index_"+marque.id)
